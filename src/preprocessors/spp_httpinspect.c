...
{   
    ...
    if (buffer)
    {
        ...
        LogMessage(" Memory Statistics of Http Inspect on: %s ",ctime(&curr_time));
        LogMessage("    Current active session          : %lu", hi_stats.session_count);
        LogMessage("    No of POST methods encountered  : %lu", hi_stats.post);
        LogMessage("    No of GET methods encountered   : %lu", hi_stats.get);
        LogMessage("    No of successfully extract post params      : %lu", hi_stats.post_params);
        LogMessage("    No of successfully extract request params   : %lu", hi_stats.req_headers);
        LogMessage("    No of successfully extract response params  : %lu", hi_stats.resp_headers);
        LogMessage(" Http Memory Pool       :");
        LogMessage("      Free Memory:    %14zu bytes ", hi_get_free_mempool(http_mempool));
        LogMessage("      Used Memory:    %14zu bytes ", hi_get_used_mempool(http_mempool));
        LogMessage("      Max Memory :    %14zu bytes ", hi_get_max_mempool(http_mempool));
        LogMessage(" Mime Decode Memory Pool   :");
        LogMessage("      Free Memory:    %14zu bytes", hi_get_free_mempool(mime_decode_mempool));
        LogMessage("      Used Memory:    %14zu bytes", hi_get_used_mempool(mime_decode_mempool));
        LogMessage("      Max Memory :    %14zu bytes", hi_get_max_mempool(mime_decode_mempool));
        LogMessage(" Http Gzip Memory Pool     :");
        LogMessage("      Free Memory:    %14zu bytes", hi_get_free_mempool(hi_gzip_mempool));
        LogMessage("      Used Memory:    %14zu bytes", hi_get_used_mempool(hi_gzip_mempool));
        LogMessage("      Max Memory :    %14zu bytes", hi_get_max_mempool(hi_gzip_mempool));
        LogMessage(" Http Mime log Memory Pool :");
        LogMessage("      Free Memory:    %14zu bytes", hi_get_free_mempool(mime_log_mempool));
        LogMessage("      Used Memory:    %14zu bytes", hi_get_used_mempool(mime_log_mempool));
        LogMessage("      Max Memory :    %14zu bytes", hi_get_max_mempool(mime_log_mempool));
